...
> 알림톡: 위 3개 + `SOLAPI_KAKAO_PF_ID` + 템플릿 ID(`..._QUOTE`/`..._BID`)까지 설정되면 **알림톡 우선·실패 시 SMS 자동 대체**. 템플릿 변수 — 견적: `#{이사종류}#{출발지}#{도착지}#{이사예정일}#{이사규모}#{견적방식}`(예정일·규모 미입력 시 '미정' 자동), 입찰: `#{업체명}#{금액}`. (`backend/src/messaging.ts`)
>
