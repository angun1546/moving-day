...
- **공지사항**(`/notice`): 관리자 작성·수정·삭제(**`Notice` 백엔드 저장 — 고객·파트너·관리자 실시간 공유**), 작성 시 알림 푸시. **GNB 바로 아래 가로 꽉 찬 공지 바**(최근 5건 GSAP 순환 + X 닫기 — 닫으면 새 공지 올 때까지 숨김, `localStorage` 기억). 공통 레이아웃이라 전 페이지 노출, 공지 0건이면 미표시
