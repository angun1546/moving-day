...
- **기업·관공서 이사**(`/business`): 개인 이사와 분리된 **B2B 전용 랜딩** — 이전 대상(사무실·관공서·공장·매장) 소개 + 무료 상담 CTA(견적 플로우 연결). 메인 서비스 섹션 진입 배너·GNB 메뉴로 진입. 견적 종류에 **기업·관공서 이사** 추가. *(차별점·진행절차 박스 내용은 회사 방침 확정 후 채울 예정 — 현재 공란)*
...
- **헤더 메뉴 네비**: 서비스→Hero·이용 절차→Process 섹션은 **어느 페이지에서 눌러도** 메인(`/`)으로 이동 후 해당 섹션으로 스크롤(같은 페이지면 바로 스크롤). 기업·관공서→`/business`·고객 후기→`/reviews`·FAQ→`/faq`·공지사항→`/notice`는 전용 페이지 라우트
