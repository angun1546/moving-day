...
> 알림톡: 위 3개 + `SOLAPI_KAKAO_PF_ID` + 템플릿 ID(`..._QUOTE`/`..._BID`/`..._AWARD`)까지 설정되면 **알림톡 우선·실패 시 SMS 자동 대체**. 템플릿 변수 — 견적: `#{이사종류}#{출발지}#{도착지}#{이사예정일}#{이사규모}#{견적방식}`(예정일·규모 미입력 시 '미정' 자동), 입찰: `#{업체명}#{금액}#{출발지}#{도착지}#{이사예정일}#{예상일정}`(입찰마다 견적 주인에게 발송, 빈 값은 '미정' 자동), 낙찰: `#{고객명}#{연락처}#{출발지}#{도착지}#{이사예정일}#{금액}`(낙찰 시 낙찰 파트너에게 고객 연락처 전달). (`backend/src/messaging.ts`)
>
