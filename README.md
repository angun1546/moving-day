...
| `Consult` | id, name, contact, moveType(이사종류), content(상담내용), status(접수/상담중/완료), memo(관리자 내부 메모·고객 비노출·nullable), reply(관리자 답변·nullable), hidden, authorEmail(nullable), createdAt — 상담신청(상담 도크 제출 공개·처리 관리자). 접수 시 연락처가 이메일이면 메일·전화면 SMS로 접수 확인 발송. 관리자 답변 등록 시 이메일 신청자에겐 답변 메일 발송 |
...
| GET | `/api/consults` | 상담신청 목록 **[관리자]** |
| GET | `/api/consults/mine` | 내 상담신청 목록 (회원 본인) |
| POST | `/api/consults` | 상담신청 (공개·회원/비회원, 이름·연락처·이사종류·내용 필수) — 접수 확인 이메일/SMS 발송 |
| PATCH | `/api/consults/:id` | 상담신청 상태·메모·답변·숨김 처리 **[관리자]** (**답변 새로 등록 시 이메일 신청자에게 답변 메일 발송**) |
| DELETE | `/api/consults/:id` | 상담신청 삭제 **[관리자]** |
...
6. **실시간 알림** (핵심 완료) — 서버 `Notification` 테이블 + 거래 이벤트(입찰·낙찰·거절·단계변경) 20초 폴링 알림. **SMS/카카오 알림톡(솔라피) 연동**(`backend/src/messaging.ts`) — 견적 등록 시 가입 업체에게, 입찰 등록 시 견적 주인에게 발송(키 없으면 mock 로그). **알림톡 7종(견적·입찰·낙찰·거절·단계·불편사항답변·FAQ답변) 카카오 검수 완료·운영 적용**(2026-06-11) — 발송프로필(`SOLAPI_KAKAO_PF_ID`)+템플릿 ID 7개 서버 적용, 알림톡 우선·실패 시 SMS 자동 대체. 향후 웹소켓·서비스 지역 매칭으로 확장
...
9. **보안·무결성 점검 + 고객 지원 라인** (완료) — **버그 점검 수정**: 파트너 입찰 업체명을 서버 프로필에서 조회(입찰 불가 버그), 낙찰 `accept`/`cancel` 상태 가드+`$transaction` 원자화, **인증 게이트**(`requireAuth`/`requireAdmin` — notices·reviews·qna·stories·projects·vlogs 작성/수정/삭제 관리자 전용, partners 프로필 본인 소유권, JWT 시크릿 원격 DB 강제, 프론트 `authHeaders` Bearer 첨부), `AuthContext.ready` 깜빡임 제거. **상단 헤더 메뉴 개편**: 기업소개·기업문화·인증현황 → **불편사항 접수·팁 게시판·FAQ·후기·공지사항**(유저·파트너·관리자). **불편사항 접수**(`Complaint`: 제출 공개·관리자 상태/답변 처리, `/complaint`)·**팁 게시판**(`Tip`: 조회 공개·관리자 CRUD, `/tips`) 풀스택 신규. **관리자 대시보드에 불편사항·팁 게시판 탭 추가**(상태 처리·답변·CRUD + "미처리 불편사항" 통계), **마이페이지(유저·파트너) 활동내역에 "내 불편사항" 카드**(`/api/complaints/mine/:email`) + ComplaintPage 로그인 사용자 본인 접수 내역 표시. **불편사항 숨김**(`Complaint.hidden` — 관리자가 숨기면 작성자 '내 불편사항'에서 제외) + 불편사항·팁 게시판 목록 **페이지네이션**(5개씩, `usePagination`)
