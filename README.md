...
10. **UX·운영 개선** (완료) — 회원정보 수정 저장 시 **마이페이지로 자동 이동**(`AccountEditPage`), **`DatePicker` 직접 타이핑 입력**(달력 선택 + `YYYY-MM-DD` 자동 포맷, 회원가입 생년월일 등 공용 적용), **운영 스크립트**(`npm run reset:admin`·`clear:content`·`test:sms` — `DATABASE_URL` 기준 동작), 배포 정리(Vercel·Render·**Turso/libSQL 제거 → better-sqlite3 단일 어댑터**, 도메인 메타 `themovingday.com` 교정), 솔라피 알림톡 변수 키 `#{}` 규격 수정
