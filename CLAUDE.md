...
필수: 모든 디자인과 코드는 반응형(모바일 퍼스트)으로 작성한다. 대부분의 모바일 기기에서 가독성이 좋아야 하고, 모든 기능이 정상 동작해야 한다. 모바일에서 잘리거나 못 누르는 요소가 없도록 sm/md/lg 브레이크포인트를 적극 활용한다.
필수: 모든 작업 내용은 푸시할 때마다 README.md에 함께 반영한다(기능·구조·API·데이터·로드맵). 푸시 직전 README가 현재 상태와 일치하는지 확인한다.
